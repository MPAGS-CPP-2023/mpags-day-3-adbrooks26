...
bool processCommandLine(const std::vector<std::string>& cmdLineArgs, ProgramSettings& cmdLineInputs)
{
    ...
    for (std::size_t i{1}; i < nCmdLineArgs; ++i) {
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            cmdLineInputs.helpRequested = true;
            break;
        } else if (cmdLineArgs[i] == "--version") {
            // Set the indicator and terminate the loop
            cmdLineInputs.versionRequested = true;
            break;
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                cmdLineInputs.inputFile = cmdLineArgs[i + 1];
                ++i;
            }
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                // Got filename, so assign value and advance past it
                cmdLineInputs.outputFile = cmdLineArgs[i + 1];
                ++i;
            }
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                cmdLineInputs.cipherKey = cmdLineArgs[i + 1];
                ++i;
            }
        } else if (cmdLineArgs[i] == "--encrypt") {
            cmdLineInputs.encryptionMode = CipherMode::Encrypt;
        } else if (cmdLineArgs[i] == "--decrypt") {
            cmdLineInputs.encryptionMode = CipherMode::Decrypt;
        } else {
