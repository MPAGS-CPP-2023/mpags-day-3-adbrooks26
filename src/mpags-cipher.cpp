...
int main(int argc, char* argv[])
{
    ...
    ProgramSettings cmdLineInputs;

    // Process command line arguments
    const bool cmdLineStatus{
        processCommandLine(cmdLineArgs,cmdLineInputs)};
    ...
    if (cmdLineInputs.helpRequested) {
    ...
    if (cmdLineInputs.versionRequested) {
    ...
    if (!cmdLineInputs.inputFile.empty()) {
        // Open the file and check that we can read from it
        std::ifstream inputStream{cmdLineInputs.inputFile};
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << cmdLineInputs.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    CaesarCipher cipher {cmdLineInputs.cipherKey};

    // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
    std::string outputText{cipher.applyCipher(inputText,cmdLineInputs.encryptionMode)};

    // Output the encrypted/decrypted text to stdout/file
    if (!cmdLineInputs.outputFile.empty()) {
        // Open the file and check that we can write to it
        std::ofstream outputStream{cmdLineInputs.outputFile};
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << cmdLineInputs.outputFile << "'" << std::endl;
            return 1;
        }
